...
export default class RegistrarMineralNoMetalico extends React.Component {  
	state = {
        nombre: '',
        uso: '',
        descripcion: '',
        ...
        /*if (this.props.match.params.accion !== 'CR'){
            ...
            axios.get(`http://localhost:3000/getMineralNoMetalicoById/${this.props.match.params.id}`, config)
                .then((res) => {
                    ...
                    this.setState({ uso: res.data[0].uso });
                }).catch((e) => {
                    console.log('Error en axios')
                })
        }*/
    ...
    renderOptions = (indexF) => {
    	...
    	return (this.state.presentaciones.map((optionPre, index) => {
    		...
    		if (existe === 0){
    			...
    			return(<option value={index}>{optionPre.nombre}</option>)
    		}
    	}));
    }
